...
export const generateNewVoucher = async () => {
  ...
  const randomChars = (length: number) => Math.random().toString(36).substring(2, 2 + length).toUpperCase();
  ...
  const newVoucher = new Voucher({ code, discountAmount, expiresAt });
